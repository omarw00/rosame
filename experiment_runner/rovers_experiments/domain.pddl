...
(:action communicate_soil_data
 ...
 :precondition (and (at ?a ?d)(at_lander ?b ?e)(have_soil_analysis ?a ?c)
                   (visible ?d ?e)(available ?a)(channel_free ?b))
 :effect (and (not (available ?a))(not (channel_free ?b))(channel_free ?b)
		(communicated_soil_data ?c)(available ?a)))

(:action communicate_rock_data
 :parameters (?a - rover ?b - lander ?c - waypoint ?d - waypoint ?e - waypoint)
 :precondition (and (at ?a ?d)(at_lander ?b ?e)(have_rock_analysis ?a ?c)
                   (visible ?d ?e)(available ?a)(channel_free ?b))
 :effect (and (not (available ?a))(not (channel_free ?b))
	      (channel_free ?b)(communicated_rock_data ?c)(available ?a)))


(:action communicate_image_data
 :parameters (?a - rover ?b - lander ?c - objective ?d - mode
	      ?e - waypoint ?f - waypoint)
 :precondition (and (at ?a ?e)(at_lander ?b ?f)(have_image ?a ?c ?d)
