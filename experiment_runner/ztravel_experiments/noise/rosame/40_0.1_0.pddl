...
(:action fly
	...
	:effect (and (at ?a ?b) (fuel-level ?a ?e)))

(:action zoom
	:parameters (?a - aircraft ?b - city ?c - city ?d - flevel ?e - flevel ?f - flevel)
	:precondition (and (at ?a ?b))
	:effect (and (at ?a ?c) (fuel-level ?a ?d) (fuel-level ?a ?e) (fuel-level ?a ?f) (next ?d ?e) (next ?d ?f) (next ?e ?d) (next ?e ?f) (next ?f ?d) (next ?f ?e) (not (at ?a ?b))))

(:action refuel
	:parameters (?a - aircraft ?b - city ?c - flevel ?d - flevel)
	:precondition (and (fuel-level ?a ?c) (fuel-level ?a ?d) (next ?c ?d) (next ?d ?c))
	:effect (and  (not (fuel-level ?a ?c))  (not (fuel-level ?a ?d))  (not (next ?c ?d))  (not (next ?d ?c))))
