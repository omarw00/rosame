...
(:action board-truck
	...
	:effect (and (at ?c ?b) (not (at ?a ?b))  (not (driving ?a ?c))))
...
(:action drive-truck
	...
	:precondition (and (at ?a ?c) (link ?c ?b) (path ?b ?c))
	:effect (and (at ?d ?c)))

(:action walk
	:parameters (?a - driver ?b - location ?c - location)
	:precondition (and (at ?a ?b) (link ?b ?c))
	:effect (and (at ?a ?c) (link ?c ?b) (path ?b ?c) (path ?c ?b) (not (at ?a ?b))))
