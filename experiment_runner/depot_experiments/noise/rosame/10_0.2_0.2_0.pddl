...
(:action load
	...
	:precondition (and (at ?a ?c) (at ?b ?c) (at ?d ?c) (on ?a ?a) (in ?a ?d) (lifting ?a ?b) (clear ?a))
	:effect (and (available ?b) (not (at ?a ?c))  (not (at ?b ?c))  (not (at ?d ?c))  (not (on ?a ?a))  (not (in ?a ?d))  (not (lifting ?a ?b))  (not (clear ?a))))

(:action unload
	:parameters (?a - crate ?b - hoist ?c - place ?d - truck)
	:precondition (and (at ?a ?c) (in ?a ?d) (clear ?a))
	:effect (and  (not (at ?a ?c))  (not (in ?a ?d))  (not (clear ?a))))
