...
(:action load
	...
	:precondition (and (at ?a ?c) (at ?d ?c) (on ?a ?a) (in ?a ?d) (available ?b) (clear ?a))
	:effect (and ))

(:action unload
	:parameters (?a - crate ?b - hoist ?c - place ?d - truck)
	:precondition (and (at ?d ?c) (in ?a ?d) (lifting ?a ?b))
	:effect (and  (not (at ?d ?c))  (not (in ?a ?d))  (not (lifting ?a ?b))))
