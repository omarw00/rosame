...
(:action drop
	:parameters (?a - crate ?b - hoist ?c - place ?d - surface)
	:precondition (and (at ?b ?c) (on ?a ?a) (on ?a ?d))
	:effect (and (at ?a ?c) (lifting ?a ?b) (available ?b) (clear ?a) (clear ?d) (not (on ?a ?d))))
...
(:action unload
	...
	:effect (and  (not (at ?a ?c))  (not (on ?a ?a))  (not (in ?a ?d))  (not (lifting ?a ?b))  (not (available ?b))  (not (clear ?a))))
