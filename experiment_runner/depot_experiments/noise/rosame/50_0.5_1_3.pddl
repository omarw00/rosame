...
(:action drop
	:parameters (?a - crate ?b - hoist ?c - place ?d - surface)
	:precondition (and (at ?b ?c) (lifting ?a ?b) (available ?b))
	:effect (and (at ?a ?c) (on ?a ?d) (clear ?a) (clear ?d) (not (lifting ?a ?b))  (not (available ?b))))

(:action load
	:parameters (?a - crate ?b - hoist ?c - place ?d - truck)
	:precondition (and (at ?a ?c) (lifting ?a ?b) (available ?b))
