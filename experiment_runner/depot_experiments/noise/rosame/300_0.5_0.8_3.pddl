...
(:action drop
	...
	:effect (and (at ?a ?c) (at ?d ?c) (on ?a ?a) (on ?a ?d) (available ?b) (clear ?a) (not (lifting ?a ?b))  (not (clear ?d))))

(:action load
	:parameters (?a - crate ?b - hoist ?c - place ?d - truck)
	:precondition (and (at ?b ?c) (lifting ?a ?b))
	:effect (and (in ?a ?d) (available ?b) (not (lifting ?a ?b))))

(:action unload
	:parameters (?a - crate ?b - hoist ?c - place ?d - truck)
	:precondition (and (at ?b ?c) (in ?a ?d) (available ?b))
