...
(:action drop
	:parameters (?a - crate ?b - hoist ?c - place ?d - surface)
	:precondition (and (at ?b ?c) (at ?d ?c) (on ?a ?a) (lifting ?a ?b) (clear ?d))
	:effect (and (at ?a ?c) (on ?a ?d) (available ?b) (clear ?a) (not (on ?a ?a))  (not (lifting ?a ?b))  (not (clear ?d))))

(:action load
	:parameters (?a - crate ?b - hoist ?c - place ?d - truck)
	:precondition (and (at ?b ?c) (at ?d ?c) (lifting ?a ?b))
	:effect (and (in ?a ?d) (not (lifting ?a ?b))))

(:action unload
	:parameters (?a - crate ?b - hoist ?c - place ?d - truck)
	:precondition (and (at ?a ?c) (at ?d ?c) (in ?a ?d))
	:effect (and (lifting ?a ?b) (not (in ?a ?d))))
