...
(:action lift
	...
	:effect (and (at ?a ?c) (clear ?d) (not (on ?a ?d))  (not (available ?b))  (not (clear ?a))))

(:action drop
	:parameters (?a - crate ?b - hoist ?c - place ?d - surface)
	:precondition (and (at ?a ?c) (at ?b ?c) (clear ?d))
...
(:action load
	...
	:effect (and  (not (at ?a ?c))  (not (at ?d ?c))  (not (on ?a ?a))  (not (in ?a ?d))  (not (lifting ?a ?b))  (not (available ?b))  (not (clear ?a))))
