...
(:action drop
	:parameters (?a - crate ?b - hoist ?c - place ?d - surface)
	:precondition (and (at ?a ?c) (at ?b ?c) (on ?a ?a) (on ?a ?d) (lifting ?a ?b) (available ?b) (clear ?d))
	:effect (and  (not (at ?a ?c))  (not (at ?b ?c))  (not (on ?a ?a))  (not (on ?a ?d))  (not (lifting ?a ?b))  (not (available ?b))  (not (clear ?d))))

(:action load
	:parameters (?a - crate ?b - hoist ?c - place ?d - truck)
	:precondition (and (at ?a ?c) (at ?b ?c) (at ?d ?c) (available ?b) (clear ?a))
	:effect (and (on ?a ?a) (in ?a ?d) (lifting ?a ?b)))
