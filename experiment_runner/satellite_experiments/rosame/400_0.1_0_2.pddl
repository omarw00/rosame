...
(:action calibrate
	...
	:precondition (and (on_board ?b ?c) (pointing ?a ?c))
	:effect (and (power_avail ?c) (power_on ?b) (calibrated ?b) (calibration_target ?a ?b) (not (on_board ?b ?c))  (not (pointing ?a ?c))))

(:action take_image
	:parameters (?a - direction ?b - instrument ?c - mode ?d - satellite)
	:precondition (and (on_board ?b ?d) (supports ?b ?c) (pointing ?a ?d) (power_on ?b) (calibration_target ?a ?b))
	:effect (and (power_avail ?d) (calibrated ?b) (have_image ?a ?c) (not (on_board ?b ?d))  (not (supports ?b ?c))  (not (pointing ?a ?d))  (not (power_on ?b))  (not (calibration_target ?a ?b))))
