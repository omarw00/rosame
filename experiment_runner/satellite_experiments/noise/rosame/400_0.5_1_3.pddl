...
(:action calibrate
	...
	:precondition (and (power_on ?b))
	:effect (and (on_board ?b ?c) (pointing ?a ?c) (power_avail ?c) (calibrated ?b) (calibration_target ?a ?b)))

(:action take_image
	:parameters (?a - direction ?b - instrument ?c - mode ?d - satellite)
	:precondition (and (pointing ?a ?d) (power_avail ?d) (calibration_target ?a ?b))
	:effect (and (on_board ?b ?d) (supports ?b ?c) (power_on ?b) (calibrated ?b) (have_image ?a ?c) (not (pointing ?a ?d))  (not (power_avail ?d))  (not (calibration_target ?a ?b))))
