...
(:action calibrate
	...
	:effect (and (on_board ?b ?c) (calibrated ?b) (not (pointing ?a ?c))  (not (calibration_target ?a ?b))))

(:action take_image
	:parameters (?a - direction ?b - instrument ?c - mode ?d - satellite)
	:precondition (and (power_avail ?d))
	:effect (and (on_board ?b ?d) (supports ?b ?c) (pointing ?a ?d) (power_on ?b) (calibrated ?b) (have_image ?a ?c) (calibration_target ?a ?b) (not (power_avail ?d))))
