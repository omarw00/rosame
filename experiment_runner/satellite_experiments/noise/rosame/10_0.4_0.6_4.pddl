...
(:action calibrate
	...
	:precondition (and (calibrated ?b))
	:effect (and (on_board ?b ?c) (power_on ?b)))

(:action take_image
	:parameters (?a - direction ?b - instrument ?c - mode ?d - satellite)
	:precondition (and (supports ?b ?c) (pointing ?a ?d) (power_on ?b))
	:effect (and (on_board ?b ?d) (power_avail ?d) (calibrated ?b) (have_image ?a ?c) (calibration_target ?a ?b)))
