...
(:action calibrate
	...
	:precondition (and (power_avail ?c) (calibrated ?b))
	:effect (and ))

(:action take_image
	:parameters (?a - direction ?b - instrument ?c - mode ?d - satellite)
	:precondition (and (pointing ?a ?d) (power_avail ?d) (have_image ?a ?c))
	:effect (and  (not (pointing ?a ?d))  (not (power_avail ?d))  (not (have_image ?a ?c))))
