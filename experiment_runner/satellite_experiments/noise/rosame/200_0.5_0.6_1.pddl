...
(:action calibrate
	...
	:effect (and (on_board ?b ?c) (power_avail ?c) (power_on ?b) (calibrated ?b) (calibration_target ?a ?b) (not (pointing ?a ?c))))

(:action take_image
	:parameters (?a - direction ?b - instrument ?c - mode ?d - satellite)
	:precondition (and (on_board ?b ?d) (power_on ?b) (calibrated ?b) (calibration_target ?a ?b))
	:effect (and (supports ?b ?c) (pointing ?a ?d) (power_avail ?d) (have_image ?a ?c) (not (on_board ?b ?d))  (not (power_on ?b))  (not (calibrated ?b))  (not (calibration_target ?a ?b))))
