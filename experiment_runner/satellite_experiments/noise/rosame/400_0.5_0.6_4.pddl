...
(:action calibrate
	...
	:precondition (and (on_board ?b ?c) (power_avail ?c) (calibration_target ?a ?b))
	:effect (and (pointing ?a ?c) (power_on ?b) (calibrated ?b) (not (on_board ?b ?c))  (not (power_avail ?c))  (not (calibration_target ?a ?b))))

(:action take_image
	:parameters (?a - direction ?b - instrument ?c - mode ?d - satellite)
	:precondition (and (pointing ?a ?d) (power_avail ?d))
	:effect (and (on_board ?b ?d) (supports ?b ?c) (power_on ?b) (calibrated ?b) (have_image ?a ?c) (calibration_target ?a ?b)))
