...
(:action switch_on
	...
	:precondition (and )
	:effect (and (on_board ?a ?b) (power_avail ?b) (calibrated ?a)))

(:action switch_off
	:parameters (?a - instrument ?b - satellite)
	:precondition (and (on_board ?a ?b) (power_avail ?b) (power_on ?a))
	:effect (and  (not (on_board ?a ?b))  (not (power_avail ?b))  (not (power_on ?a))))
...
(:action take_image
	...
	:precondition (and (calibrated ?b) (have_image ?a ?c) (calibration_target ?a ?b))
	:effect (and  (not (calibrated ?b))  (not (have_image ?a ?c))  (not (calibration_target ?a ?b))))
