...
(:action switch_off
	:parameters (?a - instrument ?b - satellite)
	:precondition (and )
	:effect (and (on_board ?a ?b) (power_avail ?b) (power_on ?a) (calibrated ?a)))

(:action calibrate
	:parameters (?a - direction ?b - instrument ?c - satellite)
	:precondition (and (on_board ?b ?c) (pointing ?a ?c) (power_avail ?c) (calibration_target ?a ?b))
	:effect (and (power_on ?b) (calibrated ?b) (not (on_board ?b ?c))  (not (pointing ?a ?c))  (not (power_avail ?c))  (not (calibration_target ?a ?b))))

(:action take_image
	:parameters (?a - direction ?b - instrument ?c - mode ?d - satellite)
	:precondition (and (power_avail ?d) (calibrated ?b) (have_image ?a ?c) (calibration_target ?a ?b))
	:effect (and  (not (power_avail ?d))  (not (calibrated ?b))  (not (have_image ?a ?c))  (not (calibration_target ?a ?b))))
