...
(:action calibrate
	...
	:effect (and  (not (on_board ?b ?c))  (not (pointing ?a ?c))))

(:action take_image
	:parameters (?a - direction ?b - instrument ?c - mode ?d - satellite)
	:precondition (and (calibrated ?b))
	:effect (and (on_board ?b ?d) (supports ?b ?c) (pointing ?a ?d) (power_avail ?d) (power_on ?b) (have_image ?a ?c) (calibration_target ?a ?b) (not (calibrated ?b))))
