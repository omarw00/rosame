...
(:action calibrate
	...
	:effect (and  (not (calibrated ?b))  (not (calibration_target ?a ?b))))

(:action take_image
	:parameters (?a - direction ?b - instrument ?c - mode ?d - satellite)
	:precondition (and (on_board ?b ?d) (power_avail ?d) (calibration_target ?a ?b))
	:effect (and  (not (on_board ?b ?d))  (not (power_avail ?d))  (not (calibration_target ?a ?b))))
