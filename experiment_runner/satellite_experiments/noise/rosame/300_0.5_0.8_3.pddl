...
(:action calibrate
	...
	:precondition (and (power_avail ?c) (power_on ?b) (calibration_target ?a ?b))
	:effect (and (on_board ?b ?c) (pointing ?a ?c) (calibrated ?b)))
...
(:action take_image
	...
	:effect (and  (not (on_board ?b ?d))  (not (supports ?b ?c))  (not (power_on ?b))  (not (calibrated ?b))  (not (have_image ?a ?c))))
