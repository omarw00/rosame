...
(:action calibrate
	...
	:effect (and  (not (on_board ?b ?c))  (not (pointing ?a ?c))  (not (power_avail ?c))))

(:action take_image
	:parameters (?a - direction ?b - instrument ?c - mode ?d - satellite)
	:precondition (and (power_on ?b) (have_image ?a ?c))
	:effect (and (on_board ?b ?d) (supports ?b ?c) (pointing ?a ?d) (power_avail ?d) (calibrated ?b) (calibration_target ?a ?b)))
