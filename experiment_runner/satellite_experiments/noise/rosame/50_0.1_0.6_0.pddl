...
(:action turn_to
	...
	:precondition (and (pointing ?b ?c) (power_avail ?c))
...
(:action calibrate
	...
	:precondition (and (on_board ?b ?c) (power_avail ?c) (power_on ?b) (calibrated ?b) (calibration_target ?a ?b))
	:effect (and (pointing ?a ?c)))

(:action take_image
	:parameters (?a - direction ?b - instrument ?c - mode ?d - satellite)
	:precondition (and (on_board ?b ?d) (supports ?b ?c) (pointing ?a ?d) (power_avail ?d) (calibrated ?b) (have_image ?a ?c) (calibration_target ?a ?b))
	:effect (and (power_on ?b) (not (on_board ?b ?d))  (not (supports ?b ?c))  (not (pointing ?a ?d))  (not (power_avail ?d))  (not (calibrated ?b))  (not (have_image ?a ?c))  (not (calibration_target ?a ?b))))
