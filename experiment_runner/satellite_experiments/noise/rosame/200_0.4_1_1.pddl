...
(:action calibrate
	...
	:precondition (and (pointing ?a ?c) (calibration_target ?a ?b))
	:effect (and (on_board ?b ?c) (power_avail ?c) (power_on ?b) (calibrated ?b)))

(:action take_image
	:parameters (?a - direction ?b - instrument ?c - mode ?d - satellite)
	:precondition (and (on_board ?b ?d) (pointing ?a ?d) (power_on ?b) (calibrated ?b) (have_image ?a ?c) (calibration_target ?a ?b))
	:effect (and  (not (on_board ?b ?d))  (not (pointing ?a ?d))  (not (power_on ?b))  (not (calibrated ?b))  (not (have_image ?a ?c))  (not (calibration_target ?a ?b))))
