...
(:action calibrate
	...
	:precondition (and )
	:effect (and (power_on ?b)))

(:action take_image
	:parameters (?a - direction ?b - instrument ?c - mode ?d - satellite)
	:precondition (and (on_board ?b ?d) (pointing ?a ?d) (power_avail ?d) (power_on ?b) (have_image ?a ?c))
	:effect (and  (not (on_board ?b ?d))  (not (pointing ?a ?d))  (not (power_avail ?d))  (not (power_on ?b))  (not (have_image ?a ?c))))
