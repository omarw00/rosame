...
(:action turn_to
	...
	:effect (and (pointing ?a ?c) (not (pointing ?b ?c))  (not (power_avail ?c))))
...
(:action calibrate
	...
	:precondition (and )
	:effect (and (power_avail ?c) (power_on ?b) (calibrated ?b) (calibration_target ?a ?b)))

(:action take_image
	:parameters (?a - direction ?b - instrument ?c - mode ?d - satellite)
	:precondition (and (on_board ?b ?d) (pointing ?a ?d) (power_avail ?d) (power_on ?b) (calibrated ?b) (have_image ?a ?c) (calibration_target ?a ?b))
	:effect (and ))
