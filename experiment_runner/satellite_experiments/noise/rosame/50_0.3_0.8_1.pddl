...
(:action switch_off
	...
	:effect (and  (not (power_avail ?b))  (not (calibrated ?a))))
...
(:action take_image
	...
	:precondition (and (pointing ?a ?d) (calibrated ?b) (have_image ?a ?c) (calibration_target ?a ?b))
	:effect (and (on_board ?b ?d) (supports ?b ?c) (power_avail ?d) (power_on ?b) (not (pointing ?a ?d))  (not (calibrated ?b))  (not (have_image ?a ?c))  (not (calibration_target ?a ?b))))
