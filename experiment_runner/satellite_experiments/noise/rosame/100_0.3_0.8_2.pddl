...
(:action calibrate
	...
	:effect (and  (not (on_board ?b ?c))  (not (pointing ?a ?c))  (not (power_avail ?c))  (not (power_on ?b))  (not (calibrated ?b))))

(:action take_image
	:parameters (?a - direction ?b - instrument ?c - mode ?d - satellite)
	:precondition (and (on_board ?b ?d) (supports ?b ?c) (pointing ?a ?d) (calibrated ?b))
	:effect (and (power_avail ?d) (power_on ?b) (have_image ?a ?c) (calibration_target ?a ?b) (not (on_board ?b ?d))  (not (supports ?b ?c))  (not (pointing ?a ?d))  (not (calibrated ?b))))
