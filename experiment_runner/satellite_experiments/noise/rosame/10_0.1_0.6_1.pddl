...
(:action switch_off
	...
	:effect (and (power_on ?a) (not (calibrated ?a))))
...
(:action calibrate
	...
	:effect (and  (not (calibrated ?b))))

(:action take_image
	:parameters (?a - direction ?b - instrument ?c - mode ?d - satellite)
	:precondition (and (calibrated ?b) (have_image ?a ?c))
	:effect (and  (not (calibrated ?b))  (not (have_image ?a ?c))))
