...
(:action calibrate
	...
	:precondition (and (on_board ?b ?c) (pointing ?a ?c) (power_avail ?c) (calibrated ?b) (calibration_target ?a ?b))
	:effect (and  (not (on_board ?b ?c))  (not (pointing ?a ?c))  (not (power_avail ?c))  (not (calibrated ?b))  (not (calibration_target ?a ?b))))

(:action take_image
	:parameters (?a - direction ?b - instrument ?c - mode ?d - satellite)
	:precondition (and (pointing ?a ?d))
	:effect (and (on_board ?b ?d) (supports ?b ?c) (power_avail ?d) (power_on ?b) (calibrated ?b) (have_image ?a ?c) (calibration_target ?a ?b)))
