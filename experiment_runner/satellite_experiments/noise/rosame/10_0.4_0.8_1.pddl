...
(:action calibrate
	...
	:precondition (and (pointing ?a ?c) (power_avail ?c) (power_on ?b) (calibrated ?b))
	:effect (and  (not (pointing ?a ?c))  (not (power_avail ?c))  (not (power_on ?b))  (not (calibrated ?b))))

(:action take_image
	:parameters (?a - direction ?b - instrument ?c - mode ?d - satellite)
	:precondition (and (on_board ?b ?d) (pointing ?a ?d) (calibrated ?b) (have_image ?a ?c))
	:effect (and (supports ?b ?c) (power_avail ?d) (power_on ?b) (calibration_target ?a ?b) (not (on_board ?b ?d))  (not (pointing ?a ?d))  (not (calibrated ?b))  (not (have_image ?a ?c))))
