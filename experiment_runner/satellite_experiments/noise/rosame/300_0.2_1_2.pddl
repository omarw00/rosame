...
(:action calibrate
	...
	:precondition (and (on_board ?b ?c) (power_avail ?c) (power_on ?b))
	:effect (and  (not (on_board ?b ?c))  (not (power_avail ?c))  (not (power_on ?b))))

(:action take_image
	:parameters (?a - direction ?b - instrument ?c - mode ?d - satellite)
	:precondition (and (on_board ?b ?d) (supports ?b ?c) (pointing ?a ?d) (power_avail ?d) (power_on ?b) (have_image ?a ?c) (calibration_target ?a ?b))
	:effect (and  (not (on_board ?b ?d))  (not (supports ?b ?c))  (not (pointing ?a ?d))  (not (power_avail ?d))  (not (power_on ?b))  (not (have_image ?a ?c))  (not (calibration_target ?a ?b))))
