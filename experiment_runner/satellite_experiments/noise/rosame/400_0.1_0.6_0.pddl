...
(:action switch_on
	...
	:effect (and (on_board ?a ?b) (power_on ?a) (not (power_avail ?b))))
...
(:action calibrate
	...
	:precondition (and (power_avail ?c) (power_on ?b) (calibrated ?b) (calibration_target ?a ?b))
	:effect (and  (not (power_avail ?c))  (not (power_on ?b))  (not (calibrated ?b))  (not (calibration_target ?a ?b))))

(:action take_image
	:parameters (?a - direction ?b - instrument ?c - mode ?d - satellite)
	:precondition (and )
	:effect (and (supports ?b ?c) (pointing ?a ?d) (power_avail ?d) (power_on ?b) (calibrated ?b) (have_image ?a ?c)))
