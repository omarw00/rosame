...
(:action calibrate
	...
	:effect (and  (not (pointing ?a ?c))  (not (calibrated ?b))))

(:action take_image
	:parameters (?a - direction ?b - instrument ?c - mode ?d - satellite)
	:precondition (and (pointing ?a ?d))
	:effect (and (on_board ?b ?d) (supports ?b ?c) (power_avail ?d) (power_on ?b) (calibrated ?b) (have_image ?a ?c) (calibration_target ?a ?b) (not (pointing ?a ?d))))
