...
(:action calibrate
	...
	:precondition (and )
	:effect (and (on_board ?b ?c) (pointing ?a ?c) (power_on ?b) (calibration_target ?a ?b)))
...
(:action take_image
	...
	:effect (and  (not (on_board ?b ?d))  (not (supports ?b ?c))  (not (pointing ?a ?d))  (not (power_avail ?d))  (not (calibrated ?b))  (not (have_image ?a ?c))  (not (calibration_target ?a ?b))))
