...
(:action drive-truck
	...
	(<= (* (time-to-drive ?loc-to ?loc-from) -1) -17)
	(<= (+ (* (driven ) 0.3850) (* (time-to-drive ?loc-to ?loc-from) -0.9229)) 110.6069)
	(<= (+ (* (time-to-drive ?loc-to ?loc-from) -0.9922) (* (driven ) -0.1249)) -53.5774)
	(<= (+ (* (time-to-drive ?loc-to ?loc-from) 0.9010) (* (driven ) -0.4338)) 48.6541)
	(<= (time-to-drive ?loc-to ?loc-from) 80)
